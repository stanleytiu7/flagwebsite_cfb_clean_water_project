...
require_once('archive_repeatingcode_2015_updated.php');//2015 update
date_default_timezone_set('America/New_York');
$specialmsg = "" ;
$corrections = 0 ;

function archive_wqmodel_2015_updated() {
		...
		$return_val = TRUE ;
			...
			if (archive_OnsetU30("CommunityBoating") == FALSE) {
				$return_val = FALSE ;
				if ($specialmsg == "") {$specialmsg = " Error in (archive_OnsetU30(Community Boating).";}
			}
		...
		if ($return_val==TRUE) {
			echo "Archiving job finished successfully." ;
			echo $specialmsg ;
...
function archive_boatingmodel() {
		...
		$rainset = array() ;
		$daysset = array() ; //
		$hoursset = array();//2015 update - Hours Since Rain 0.2 inches
		$windset = array() ;
		$flowset = array() ;
		$parset = array() ;
		$cso_CP_set = array() ;
		...
		//update 2015 
		$statement = $wqdb->query("SELECT datetime,watertemp_C,airtemp_C,rain_in,raindays,rainhours,windspeed_mph,flow_cfs,par_uE,cso_CP,cyano_NewtonYC,cyano_WatertownYC,cyano_CommRowing,cyano_CRCK,cyano_HarvardWeld,cyano_RiversideBC,cyano_CRYC,cyano_UnionBC,cyano_CommBoating,cyano_CRCKKendall FROM $modeltable WHERE location='$loc' AND datetime<TIMESTAMP('$stopdate') ORDER BY datetime");
		...
		while($row = $statement->fetch_assoc()){
			...
			$wtmpset[$arraycount] = $row['watertemp_C'] ; //already in Celcius
			$atmpset[$arraycount] = $row['airtemp_C'] ; //already in Celcius ? 2015 update
			$rainset[$arraycount] = $row['rain_in'] ;
			$daysset[$arraycount] = $row['raindays'] ;
			$hoursset[$arraycount] = $row ['rainhours']; // 2015 update
		...
		$starttime = max(mktime(date("G",$mdl_lastdate)+1, 0, 0, date("m",$mdl_lastdate), date("d",$mdl_lastdate), 
			date("Y",$mdl_lastdate)), mktime(0, 0, 0, date("m"), date("d")-$mdl_recalc-100, date("Y")));
		$endtime = mktime(date("G"), 0, 0, date("m"), date("d"), date("Y")) ;
		$startcount = $arraycount ;
		for ($i=$startcount;$i<=$startcount + round(($endtime - $starttime) / (60*60));$i++) { //what time unit?
			$timeset[$i] = $starttime + ($i-$startcount) * (60*60) ; //what time unit?
			//following required to make sure arrays are all of same offset for array_slice()
			$wtmpset[$i] = NULL ;
			$atmpset[$i] = NULL; //update 2015
			$rainset[$i] = NULL ;
			$daysset[$i] = NULL ;
			$hoursset[$i] = NULL ; //update 2015
			$windset[$i] = NULL ;
			$flowset[$i] = NULL ;
			$parset[$i] = NULL ;
			$cso_CP_set[$i] = 0 ;
		...
		// 2015 update: should to the equivalent to days for hours but I'm not sure how to do because it depends on time units... test
		if ($startcount==0) {
			$hours = 0 ;
		} else {
			$hours = $hoursset[$startcount-1] ;
		}
				...
				if ($thishour == $trgthour) {
					...
					if ($trgthour > mktime(0,0,0,0,0,0)) {
						...
						if ($indx !== FALSE) {
							...
							// update 2015 For Hours Since Rain 0.2 - test
							if(array_sum(array_slice($rainset,$indx-1,2))>=0.2){
								$hours=0;
							} else {
								$hours=$hours+1;
							}
		...
		if ($starttime <= $endtime) {
			...
			for($x=0;$x<$arraycount;$x++) {
				if (is_null($rainset[$x])==FALSE && is_null($windset[$x])==FALSE && is_null($wtmpset[$x])==FALSE && is_null($atmpset[$x])==FALSE &&
					is_null($flowset[$x])==FALSE && is_null($daysset[$x])==FALSE &&  is_null($hoursset[$x])==FALSE && is_null($parset[$x])==FALSE) {
					$rowcount++ ;
					...
					if ($rowcount >= 168) {
						...
						$lg_R2 =  0.2629+0.0444*$wtmpD1-0.0364*$atmpD1+0.0014*$hoursset[$x]-0.226*log($hoursset[$x]+0.0001) ;
						$lg_R3 = 1.4144+0.0255*$wtmpD1-0.0007*$parD2+0.0009*$hoursset[$x]-0.3022*log($hoursset[$x]+0.0001)+0.0015*$flowD2-0.3957*log($flowD2) ;
						$lg_R4 = 3.6513+0.0254*$wtmpD1-0.6636*log($parD2)-0.0014*$hoursset[$x]-0.3428*log($hoursset[$x]+0.0001) ;
					...
					//2015 update
					$wqdb->query("INSERT INTO $modeltable (indx,location,datetime,watertemp_C,airtemp_C,rain_in,raindays,rainhours,windspeed_mph,flow_cfs,par_uE,lg_prb_R2_pct,lg_prb_R3_pct,lg_prb_R4_pct,li_conc_LF_cfu, lg_prb_LF_pct,cso_CP,cyano_NewtonYC,cyano_WatertownYC,cyano_CommRowing,cyano_CRCK,cyano_HarvardWeld,cyano_RiversideBC,cyano_CRYC,cyano_UnionBC,cyano_CommBoating,cyano_CRCKKendall) 
					VALUES ('$rowcount','$loc',TIMESTAMP('$thistimestring'),round($wtmpset[$x],3),round($atmpset[$x],3),'$rainset[$x]',round($daysset[$x],2),round($hoursset[$x],2),'$windset[$x]','$flowset[$x]',round($parset[$x],0),round($lg_prb_R2,2),round($lg_prb_R3,2),round($lg_prb_R4,2),round($li_conc_LF),round($lg_prb_LF,2),'$cso_CP_set[$x]','$cyano_NYC_set[$x]','$cyano_WYC_set[$x]','$cyano_CR_set[$x]','$cyano_CRCK_set[$x]','$cyano_HW_set[$x]','$cyano_RBC_set[$x]','$cyano_CRYC_set[$x]','$cyano_UBC_set[$x]','$cyano_CB_set[$x]','$cyano_CRCKK_set[$x]')");
			...
			$ModReach3 = 5; // update 2015
			$ModReach4 = 6; // update 2015 
			$ModReach5 = 7; // update 2015 - Last # of locations in boathouses array that are in Model Reach 5
			...
			for ($x=0;$x<count($boathouse);$x++) {
				...
				$ModReach2Ch = TRUE ; //update 2015
				if ($x<=$ModReach2) {$ModReach2Ch = TRUE;} else {$ModReach2Ch = FALSE;}//update 2015
				
				$ModReach3Ch = TRUE ;//update 2015
				if ($x==$ModReach3) {$ModReach3Ch = TRUE;} else {$ModReach3Ch = FALSE;}//update 2015
				
				$ModReach4Ch = TRUE ;//update 2015
				if ($x==$ModReach4) {$ModReach4Ch = TRUE;} else {$ModReach4Ch = FALSE;}//update 2015
				
				$ModReach5Ch = TRUE ;//update 2015
				if ($x>=$ModReach5) {$ModReach5Ch = TRUE;} else {$ModReach5Ch = FALSE;}//update 2015
				
				
				$flaginfo = runflaglogic($ModReach2Ch,$ModReach3Ch,$ModReach4Ch,$ModReach5Ch,$final_lg_R2,$final_lg_R3,$final_lg_R4,$final_li_LF,$final_lg_LF,$boathouse_set[$x][$finalx],$cso_CP_set[$finalx],$boathouse_set[$x][$lastx],$cso_CP_set[$lastx]);
				...
				switch ($flaginfo[0]) {
					...
					case "yellow": $flagimg = "../images/yellow_flag.jpg" ; break ;
			...
			$date_handle = fopen("../backend/lastupdate.txt","w");
				fwrite($date_handle,strftime("%Y-%m-%d %H:%M:%S", $timeset[$lastx])) ;
			fclose($date_handle) ;
		}
		$wqdb->close();
		return TRUE ;
...
function read_boatingmodel($days) {  
		...
		while($row = $statement->fetch_assoc()){
			...
			$hoursset[$indx] = $row['rainhours'] ; //2015 update
		...
		$response[12] = $Log_R3_set ;//update 2015
		$response[13] = $Log_R4_set ;//update 2015
		//$response[9] = $Lin_NBBU_set ;
		//$response[11] = $Log_NBBU_set ;
		$response[14] = $Lin_LF_set ;
		$response[15] = $Log_LF_set ;
		$response[16] = $cso_CP_set ;
		$response[17] = $cyano_NYC_set ;
		$response[18] = $cyano_WYC_set ;
		$response[19] = $cyano_CR_set ;
		$response[20] = $cyano_CRCK_set ;
		$response[21] = $cyano_HW_set ;
		$response[22] = $cyano_RBC_set ;
